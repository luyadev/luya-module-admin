		    ...
		    <?= Module::t('aw_userhistorysummary_ngrestlogs'); ?>
		  	...
		  	<ul class="list-group list-group-flush">
			    ...
			    <li class="list-group-item p-0" ng-show="hiddenElement<?= $log->id; ?>">
			    	...
			    	<?php if ($log->is_insert): ?>
			    		<?php foreach ($log->attributes() as $key => $value): if (empty($value)): continue; endif; ?>
			    		<tr>
			    			<td><?= $key; ?></td>
			    			<td></td>
			    		...
			    		<?php endforeach; ?>
				    <?php elseif ($log->is_update): ?>
				    	<?php foreach ($log->attributes() as $key => $value): $oldValue = $log->getAttributeFromJsonDiffArray($key); if (empty($value) && empty($oldValue)): continue; endif; ?>
			    		<tr>
			    			<td><?= $key; ?></td>
